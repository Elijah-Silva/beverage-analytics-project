...
FROM stage.session_batch_inventory sbi
    JOIN core.vendors v ON v.vendor_name = sbi.vendor_name       -- gets vendor_id
    JOIN core.products p ON p.product_name = sbi.product_name    -- gets product_id
                        AND p.vendor_id = v.vendor_id
    JOIN core.batch_inventory bi ON bi.product_id = p.product_id
    JOIN ref.roles r ON r.role_name = sbi.role
    JOIN core.sessions s ON s.session_code = sbi.session_code;